 ...
 if(empty($_SESSION['namauser']) AND empty($_SESSION['passuser'])){
 	...
 	$query=mysqli_query($koneksi,"SELECT * FROM fasilitas WHERE id_fasilitas='$kd'");
  	while ($data=mysqli_fetch_array($query)) {
	 	...
	 	if($queryHapus){
	 		echo "<script>alert('Data Berhasil DiHapus '); window.location='$admin_url'+'adminweb.php?module=event';</script>";
	 	}else{
	 		echo "<script>alert('Gagal menghapus data '); window.location='$admin_url'+'adminweb.php?module=event';</script>";
	 	}
