...
if(empty($_SESSION['namauser']) AND empty($_SESSION['passuser'])){
	...
	if ($foto_type =='image/jpg' OR $foto_type =='image/png' OR $foto_type =='image/jpeg') {
		if ($foto_size < 5024000 ) {
		   ...
		   $querySimpan= mysqli_query($koneksi,"INSERT INTO profil (nama_profil,deskripsi,foto) VALUES ('$nama','$deskripsi','$foto')");
		   if($querySimpan){
				echo "<script>alert('Data Berhasil Disimpan '); window.location='$admin_url'+'adminweb.php?module=profil';</script>";
			}else{
				echo "<script>alert('Gagal menyimpan data '); window.location='$admin_url'+'adminweb.php?module=tambah_profile';</script>";
			}

		}else{
		   echo "<script>alert('Gagal menyimpan data, ukuran foto/gambar lebih dari 5MB '); window.location='$admin_url'+'adminweb.php?module=tambah_profil';</script>";
	   }
   }else{
