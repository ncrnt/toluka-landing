 ...
 if(empty($_SESSION['namauser']) AND empty($_SESSION['passuser'])){
 	...
 	if ($foto_type =='image/jpg' OR $foto_type =='image/png' OR $foto_type =='image/jpeg') {
 		if ($foto_size < 1024000 ) {
		    ...
		    $querySimpan= mysqli_query($koneksi,"INSERT INTO produk (nama_produk,deskripsi,foto) VALUES ('$nama','$deskripsi','$foto')");
		    if($querySimpan){
	 			echo "<script>alert('Data Berhasil Disimpan '); window.location='$admin_url'+'adminweb.php?module=produk';</script>";
		 	}else{
		 		echo "<script>alert('Gagal menyimpan data '); window.location='$admin_url'+'adminweb.php?module=tambah_produk';</script>";
		 	}
